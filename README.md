...
Version 0.2.0 closes the initial M0 feasibility gates with deliberately narrow, reproducible evidence. It is still an engineering foundation for the longer interoperability roadmap, not a complete M1 release or a claim of Word-class compatibility. See `docs/IMPLEMENTATION_STATUS.md` for the gate-by-gate evidence and `docs/COMPATIBILITY.md` for the current conservative feature matrix.
...
- View-only rendering of the bounded safe subset of internally related PNG/JPEG `wp:inline` DrawingML images at their declared OOXML extents, with oversized images reduced to the available page width. The OOXML engine can also author byte-owned internal PNG/JPEG inline images and multiple ordered sections, then reopen them with their image bytes, dimensions, names, section ranges, break kinds, and page geometry intact. Those authoring surfaces are engine/API-only in 0.2.0; the Picture command and section-editing UI do not expose them yet.
...
install -m 0644 dist/owl-docs_0.2.0_amd64.deb /tmp/owl-docs_0.2.0_amd64.deb
sudo apt install /tmp/owl-docs_0.2.0_amd64.deb
owl-docs
```
