...
Version 0.5.13 is an M1 development checkpoint plus the first complete M2
...
- Default headers and footers are edited directly on the page: click the left, center, or right region in any page's header/footer, or use Insert > Header & Footer, and type in place. Changes automatically appear on every page. The three regions retain their alignment in Owl Docs, print/PDF, and Word through native center/right tab stops; `{PAGE}` and `{PAGES}` become live page fields. Header/footer edits participate in undo/redo and recovery and save as native `word/header1.xml` and `word/footer1.xml` parts. Rich character formatting and section-specific first/even/linked variants remain future work.
...
install -m 0644 dist/owl-docs_0.5.13_amd64.deb /tmp/owl-docs_0.5.13_amd64.deb
sudo apt install /tmp/owl-docs_0.5.13_amd64.deb
owl-docs
```
