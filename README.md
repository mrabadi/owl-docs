...
- An optional `codex app-server` stdio client with runtime model/effort/service-tier discovery, streamed chat, bounded document read/search tools, and live semantic edit previews. The supported path explicitly advertises `capabilities.experimentalApi=true` during `initialize` before registering the `editor.v1` `dynamicTools` on `thread/start`, as required by the currently pinned experimental app-server schema. A fake-process controller test enforces that ordering, and an optional installed-runtime schema check verifies `experimentalApi`, `dynamicTools`, `DynamicToolSpec`, and `item/tool/call` when Codex and Python are present. This surface is experimental: an absent or incompatible runtime disables optional AI behavior without preventing offline editing. Accepted previews are one undoable transaction.
...
install -m 0644 dist/owl-docs_0.4.0_amd64.deb /tmp/owl-docs_0.4.0_amd64.deb
sudo apt install /tmp/owl-docs_0.4.0_amd64.deb
owl-docs
```
