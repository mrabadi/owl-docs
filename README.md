...
Version 0.5.12 is an M1 development checkpoint plus the first complete M2
...
- Editable default headers and footers are available from Insert > Header & Footer. They render on every page, support `{PAGE}` and `{PAGES}` fields, participate in undo/redo and recovery, and save as native `word/header1.xml` and `word/footer1.xml` parts. Rich formatting and section-specific first/even/linked variants remain future work.
...
install -m 0644 dist/owl-docs_0.5.12_amd64.deb /tmp/owl-docs_0.5.12_amd64.deb
sudo apt install /tmp/owl-docs_0.5.12_amd64.deb
owl-docs
```
