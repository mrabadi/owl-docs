...
Version 0.5.14 is an M1 development checkpoint plus the first complete M2
...
- Default headers and footers are edited directly on the page: click the left, center, or right region in any page's header/footer, or use Insert > Header & Footer, then type or paste a PNG/JPEG picture in place. Changes automatically appear on every page, and the editing controls—including their text—scale with document zoom. The three regions retain their alignment in Owl Docs, print/PDF, and Word through native center/right tab stops; `{PAGE}` and `{PAGES}` become live page fields. Header/footer text and pictures participate in undo/redo and recovery and save as native header/footer parts with DrawingML image relationships. Rich character formatting and section-specific first/even/linked variants remain future work.
...
- Local recovery schema v13 retains semantic text, run and paragraph formatting, paragraph-style identity, separate inherited text/paragraph-mark baselines, exact override provenance, durable body-paragraph insertion formats, equations, supported body and header/footer picture bytes/layout/alt text, tables, body order, default header/footer stories, identities, and page geometry. Legacy decode paths remain in place, with migration fixtures covering schema versions 3–11; fields introduced later default to safe empty, inline, or provenance-free forms when absent.
...
install -m 0644 dist/owl-docs_0.5.14_amd64.deb /tmp/owl-docs_0.5.14_amd64.deb
sudo apt install /tmp/owl-docs_0.5.14_amd64.deb
owl-docs
```
