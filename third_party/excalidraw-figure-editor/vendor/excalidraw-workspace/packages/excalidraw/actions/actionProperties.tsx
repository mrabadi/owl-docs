...
export const actionChangeFontFamily = register({
  ...
  perform: (elements, appState, value, app) => {
    ...
    if (
      (nextFontFamily || currentItemFontWeight || currentItemFontStyle) &&
      !skipOnHoverRender
    ) {
      ...
      const fontFamily = Object.entries(FONT_FAMILY).find(
        ([_, value]) =>
          value === (nextFontFamily ?? appState.currentItemFontFamily),
      )?.[0];
        ...
        elements: changeProperty(
          ...
          (oldElement) => {
            if (
              ...
              (oldElement.fontFamily !== (nextFontFamily ?? oldElement.fontFamily) ||
                oldElement.fontWeight !==
                  (currentItemFontWeight ?? oldElement.fontWeight) ||
                oldElement.fontStyle !==
                  (currentItemFontStyle ?? oldElement.fontStyle) ||
                currentItemFontFamily) // force update on selection
            ) {
              const newElement: ExcalidrawTextElement = newElementWith(
                oldElement,
                {
                  fontFamily: nextFontFamily ?? oldElement.fontFamily,
                  fontWeight: currentItemFontWeight ?? oldElement.fontWeight,
                  fontStyle: currentItemFontStyle ?? oldElement.fontStyle,
                  lineHeight: getLineHeight(
                    nextFontFamily ?? oldElement.fontFamily,
                  ),
                },
              );
      ...
      const fontString = `10px ${getFontFamilyString({
        fontFamily: nextFontFamily ?? appState.currentItemFontFamily,
      })}`;
      const chars = Array.from(uniqueChars.values()).join();

      // A weight/style change does not require loading another family.  Resize
      // the returned scene elements synchronously so the action is atomic and
      // undoable, instead of waiting for FontFaceSet (which is unreliable for
      // host-installed Helvetica and Verdana).
      if (!nextFontFamily && (currentItemFontWeight || currentItemFontStyle)) {
  ...
  PanelComponent: ({ elements, appState, app, updateData }) => {
    ...
    const selectedTextStyle = (property: "fontWeight" | "fontStyle") =>
      getFormValue(
        elements,
        appState,
        (element) => {
          ...
          return boundTextElement ? boundTextElement[property] : null;
        },
        (element) =>
          isTextElement(element) ||
    ...
    const selectedFontWeight = selectedTextStyle("fontWeight");
    const selectedFontStyle = selectedTextStyle("fontStyle");
    const supportsTextStyle = [
      FONT_FAMILY.Helvetica,
      FONT_FAMILY.Verdana,
      FONT_FAMILY.Ubuntu,
    ].includes(selectedFontFamily as FontFamilyValues);
    ...
    return (
      <fieldset>
        ...
        />
        <div className="excalidraw-local-text-style" role="group" aria-label="Text style">
          <button
            type="button"
            className={selectedFontWeight === "bold" ? "is-active" : ""}
            disabled={!supportsTextStyle}
            aria-pressed={selectedFontWeight === "bold"}
            onClick={() =>
              updateData({
                currentItemFontWeight:
                  selectedFontWeight === "bold" ? "normal" : "bold",
              })
            }
          >
            <strong>B</strong>
