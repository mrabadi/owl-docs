...
export const measureText = (
  ...
  const _text = text
    ...
    .map((x) => x || " ")
    .join("\n");
  // Font shorthands can begin with style/weight (for example,
  // "italic bold 20px Helvetica").  `parseFloat(font)` only worked when the
  // size was the first token and otherwise produced NaN dimensions.
